...
contract LiskGarden {
    ...
    function waterPlant(uint256 plantId) external {
        ...
        require(plant.exists, "Tanaman tidak ditemukan");
        require(plant.owner == msg.sender, "Bukan pemilik tanaman");
        require(!plant.isDead, "Tanaman telah mati");
    ...
    function harvestPlant(uint256 plantId) external {
        ...
        require(plant.stage == GrowthStage.BLOOMING, "Tanaman belum berbunga");
        plant.exists = false;

        uint256[] storage userPlantIds = userPlants[msg.sender];
        for (uint256 i = 0; i < userPlantIds.length; i++) {
            if (userPlantIds[i] == plantId) {
                userPlantIds[i] = userPlantIds[userPlantIds.length - 1];
                userPlantIds.pop();
                break;
            }
        }
        ...
        require(success, "Transfer hadiah gagal");
    }
